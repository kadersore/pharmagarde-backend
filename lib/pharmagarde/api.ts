...
function asRecords(payload: unknown): Record<string, unknown>[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    const possibleArrays = [record.data, record.items, record.results, record.pharmacies, record.cliniques, record.clinics, record.medicaments, record.medicines];
...
function normalizePlace(raw: Record<string, unknown>, type: "pharmacy" | "clinic", index: number): HealthPlace | null {
  ...
  const id = getString(raw, ["id", "uuid", "_id", "code", "slug"]) ?? (name ? `${type}-${name}-${index}` : undefined);
